...
class Game:
    def __init__(self, partner, stakes, starting_balance):
        ...
        # gui heading (row 0)
        self.balance_box_label = Label(self.game_box, text="Heading", font="Arial 19 bold")
        self.balance_box_label.grid(row=0)

        # display balance (row 1)
        self.balance_display = Label(self.game_box,text="Balance...")
        self.balance_display.grid(row=1)

        # button that increases balance (row 2)
        self.balance_gain = Button(self.game_box,text="Gain",command=self.
        set_balance)
        self.balance_gain.grid(row=2)

    def set_balance(self):
        self.balance.set(self.balance.get() + 2)
        self.balance_display.config(text = "Balance: {}".format(self.balance.get()))
